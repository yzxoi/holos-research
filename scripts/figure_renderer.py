#!/usr/bin/env python3
"""
FigureSpec → SVG Renderer — Deterministic architecture diagram generation.

Converts a FigureSpec JSON into publication-quality SVG for academic papers.
Deterministic: same spec = same SVG, every time.
Supports 5 built-in themes matching the plot.py theme system.

Usage:
    python3 figure_renderer.py render spec.json [--output figures/output.svg] [--theme neurips-vivid]
    python3 figure_renderer.py validate spec.json
    python3 figure_renderer.py schema
    python3 figure_renderer.py --list-themes
"""
...
# Theme palettes matching plot.py themes
THEME_PALETTES = {
    "clean-modern": {
        "palette": [
            "#4C72B0",
...
def apply_theme(spec: dict, theme_name: str) -> dict:
    """Apply a theme's palette and style to a spec that doesn't already define them."""
    if theme_name not in THEME_PALETTES:
        return spec
    theme = THEME_PALETTES[theme_name]
    style = spec.get("style", {})
    if "palette" not in style:
        style["palette"] = theme["palette"]
    if "bg_color" not in style:
        style["bg_color"] = theme["bg_color"]
    if "font_family" not in style:
        style["font_family"] = theme["font_family"]
    spec["style"] = style
    # Apply theme defaults to nodes/edges that don't override
    spec["_theme"] = theme
    return spec
...
def validate_spec(spec: dict) -> list:
    ...
    if fs is not None and (
        isinstance(fs, bool) or not isinstance(fs, (int, float)) or fs <= 0
    ):
    ...
    if pal is not None:
        if not isinstance(pal, list) or len(pal) == 0:
            issues.append(
                f"CRITICAL: style.palette must be a non-empty list of hex colors"
            )
        else:
            for pi, pc in enumerate(pal):
                if not isinstance(pc, str) or not HEX_COLOR_RE.match(pc):
                    issues.append(
                        f"CRITICAL: style.palette[{pi}] '{pc}' is not a valid hex color (#RRGGBB)"
                    )
    ...
    for i, node in enumerate(nodes):
        if not isinstance(node, dict):
            issues.append(
                f"CRITICAL: nodes[{i}] must be a dict, got {type(node).__name__}"
            )
            continue
        nid = node.get("id")
        if not nid:
        ...
        for coord in ("x", "y"):
            ...
            elif isinstance(node[coord], bool) or not isinstance(
                node[coord], (int, float)
            ):
                issues.append(
                    f"CRITICAL: node '{nid}' {coord} must be a number, got {type(node[coord]).__name__}"
                )
        for dim in ("width", "height"):
            val = node.get(dim)
            if val is not None and (
                ...
                issues.append(
                    f"CRITICAL: node '{nid}' {dim} must be a number, got {type(val).__name__}"
                )
        ...
        if shape not in ALLOWED_SHAPES:
            issues.append(
                f"WARN: node '{nid}' unknown shape '{shape}', will use 'rounded'"
            )
    ...
    for i, edge in enumerate(spec.get("edges", [])):
        ...
        if not src or not dst:
            ...
            if src not in node_ids:
                issues.append(
                    f"CRITICAL: edge[{i}] 'from' references unknown node '{src}'"
                )
            if dst not in node_ids:
                issues.append(
                    f"CRITICAL: edge[{i}] 'to' references unknown node '{dst}'"
                )
    ...
    for i, label in enumerate(spec.get("labels", [])):
        ...
        if anchor and anchor not in ALLOWED_ANCHORS:
            issues.append(
                f"WARN: label[{i}] unknown anchor '{anchor}', will use 'middle'"
            )
    ...
    for node in nodes:
        ...
        if nfs is not None and (
            isinstance(nfs, bool) or not isinstance(nfs, (int, float)) or nfs <= 0
        ):
    ...
    for i, a in enumerate(nodes):
        ...
        for j, b in enumerate(nodes):
            ...
            if abs(ax - bx) < (aw + bw) / 2 - 5 and abs(ay - by) < (ah + bh) / 2 - 5:
                issues.append(
                    f"WARN: nodes '{a.get('id')}' and '{b.get('id')}' may overlap"
                )
...
def render_svg(spec: dict) -> str:
    ...
    for i, node in enumerate(spec.get("nodes", [])):
        n = {**DEFAULT_NODE, **node}
        n["fill"] = sanitize_color(
            n.get("fill") or lighten_color(palette[i % len(palette)])
        )
        n["stroke"] = sanitize_color(n.get("stroke") or palette[i % len(palette)])
        n["text_color"] = sanitize_color(
            n.get("text_color", DEFAULT_NODE["text_color"])
        )
    ...
    for edge in spec.get("edges", []):
        ...
        if src["id"] == dst["id"]:
            ...
            top_x, top_y_pt = clip_to_shape(
                cx, cy, cx, cy - 100, src["width"], src["height"], src_shape
            )
            path_d = (
                f"M {top_x - 10},{top_y_pt} "
...
def svg_to_png(svg_path: str, png_path: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ["rsvg-convert", "-o", png_path, svg_path], capture_output=True, timeout=30
        )
...
def main():
    ...
    p_render.add_argument(
        ...
        "-t",
        default=None,
        help="Apply theme: clean-modern, nature-elegant, neurips-vivid, warm-minimal, monochrome-pro",
    )
    p_render.add_argument(
        "--preview", action="store_true", help="Also generate PNG preview"
    )
    ...
    if args.list_themes:
        ...
        for name, t in THEME_PALETTES.items():
            colors = " ".join(t["palette"][:4])
            print(f"  {name:20s} {colors} ...")
        return
    ...
    if args.command == "render":
        ...
        if args.theme:
            spec = apply_theme(spec, args.theme)
